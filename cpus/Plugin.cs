...
namespace cpus
{
        [BepInPlugin("com.kirisoup.hff.cpus", PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VERSION)]
        [BepInDependency("com.plcc.hff.timer", BepInDependency.DependencyFlags.SoftDependency)]
        [BepInProcess("Human.exe")]
        public partial class Plugin : BaseUnityPlugin
        {
                readonly Harmony harmony = new("com.kirisoup.hff.cpus");
                ...
                static void EnterCpReversed(ref int checkpoint)
                {
                        if (Game.currentLevel.nonLinearCheckpoints) return;
                        
                        // do not execute cprev logic if #cp is not changed
                        if (Game.instance.currentCheckpointNumber == checkpoint) return;
                ...
                // static void FinishValidate() => missedCp = Game.instance.passedLevel && CheckInvalid(Game.instance.currentCheckpointNumber);
